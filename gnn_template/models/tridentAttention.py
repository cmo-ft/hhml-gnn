...
class TridentAttentionConv(MessagePassing):
    _alpha: OptTensor
...
class TridentAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, heads=8, dim_head=None,
        ...
        self.mhsa = TridentAttentionConv(in_channels=in_channels, out_channels=self.dim_head, heads=heads, edge_dim=dim_edge)
...
class TridentAttentionNet(torch.nn.Module):
    def __init__(self, settings):
        ...
        for layer_idx, layer_param in enumerate(settings['conv_params']):
            ...
            self.conv_process.append(TridentAttentionBlock(in_channels=previous_output_shape,
